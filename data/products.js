...
class Product {
  ...
  rating;
  priceCents;
  constructor(productDetaills) {
    this.id = productDetaills.id;
    this.image = productDetaills.image;
    this.name = productDetaills.name;
    this.rating = productDetaills.rating;
    this.priceCents = productDetaills.priceCents;
  }
  getStarsUrl() {
...
class Clothing extends Product {
  sizeChartLink;
  constructor(productDetaills) {
    super(productDetaills);
    this.sizeChartLink = productDetaills.sizeChartLink;
  }
...
export function loadProductsFetch() {
  const promise = fetch("https://supersimplebackend.dev/products")
    ...
    .then((productsData) => {
      products = productsData.map((productDetaills) => {
        if (productDetaills.type === "clothing") {
          return new Clothing(productDetaills);
        }
        return new Product(productDetaills);
      });
      console.log("loaed products");
    }).catch(()=>{
...
export function loadProducts(fun) {
  ...
  xhr.addEventListener("load", () => {
    products = JSON.parse(xhr.response).map((productDetaills) => {
      if (productDetaills.type === "clothing") {
        return new Clothing(productDetaills);
      }
      return new Product(productDetaills);
    });
    console.log("loaed message");
    fun();
  });
